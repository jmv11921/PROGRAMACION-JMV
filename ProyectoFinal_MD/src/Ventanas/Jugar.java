...
public class Jugar extends JFrame {
	...
	private Jugador jugador; // recoge el paso por referencia desde la ventana Registro
	...
	// Variables necesarias para asignar las imágenes de los dados necesarios
	// aleatoriamente
	...
	public Jugar(Jugador j) {
		...
		JLabel_Saludo = new JLabel("Bienvenido " + jugador.getNombre() + " " + jugador.getApellido1() + " al juego");
		...
		labelSuma.setAlignmentX(Component.CENTER_ALIGNMENT);
		labelSuma.setForeground(Color.WHITE);
		labelSuma.setBounds(239, 142, 75, 75);
		getContentPane().add(labelSuma);
		ImageIcon suma = new ImageIcon(getClass().getResource("/img/suma.png")); // Creamos un objeto imageicon con la
																					// ruta de la imagen que queremos

		// ImageIcon suma = new ImageIcon(new ImageIcon("/img/suma.png")
		// .getImage().getScaledInstance(75, 75, Image.SCALE_SMOOTH));

		// File f = new File("/img/suma.png");
		// BufferedImage image = ImageIO.read(f);
		// BufferedImage ret = new BufferedImage(72,67,BufferedImage.TYPE_INT_RGB);
		// ret.getGraphics().drawImage(image,0,0,72,67,null);

		// BufferedImage ret = new BufferedImage(75,75,BufferedImage.TYPE_INT_RGB);
		// ret.getGraphics().drawImage(suma.getImage(),0,0,75,75,null);
		// ImageIcon suma1 = new ImageIcon(ret);
		...
		labelResta.setBounds(321, 142, 75, 75);
		...
		txtPuntuacion.setText(String.valueOf(jugador.getPuntuacion()));
		txtPuntuacion.setBounds(326, 79, 98, 32);
		contentPane.add(txtPuntuacion);
		txtPuntuacion.setColumns(10);

		// Botón MATHDICE
		botonVerificar = new JButton("MATHDICE");
		...
		// Botón REPITE JUGADA
		botonRepite = new JButton("REPITE JUGADA");
		...
		txtPuntuacin.setEditable(false);
		txtPuntuacin.setText("Puntuaci\u00F3n");
		txtPuntuacin.setBounds(326, 59, 98, 20);
	...
	private void inicializarDados() {
		...
		for (int i = 0; i < dados3.length; i++) {
			...
			// Solución para redimensionar las imágenes al tamaño de las cajas JLabel (no me
			// funciona en el for):
			// Icon icono= new
			// ImageIcon(dados3[i].getImage().getScaledInstance(dado[i].getIconWidth(),
			// dado[i].getIconHeight(), Image.SCALE_DEFAULT));
		...
		// Añadimos los MouseListeners a cada uno de los 5 dados con los que operamos
		// para cuando se pulse sobre estos
		...
		// Añadimos los MouseListeners a los iconos de suma y resta para cuando se pulse
		// sobre estos
		labelSuma.addMouseListener(new ListenerOperaciones());
		labelResta.addMouseListener(new ListenerOperaciones());
	...
	// JLabel_Saludo.setText("Bienvenido " + jugador.getNombre() + " al juego");
	// }
	...
	public void operacion(int num) {
		...
		} // Si no toca ni sumar ni restar, cogemos el número
	}

	// MÉTODO PARA COMPROBAR QUE LA SUMA DE TODOS SEA MAYOR O IGUAL QUE EL DADO DE
	// 12 CARAS, SI NO NO SE PUEDE JUGAR Y DEBEREMOS VOLVER A LANZAR LOS DADOS
	// De esta manera nunca ocurrirá que por ejemplo salga en todos los dados el 1,
	// y en el dado de 12 un 10, por ejemplo.
	public void sePuedeJugar() {
		...
		// Cada vez que se lancen los dados, comprobará que la suma de todos sea al
		// menos igual que el valor del de 12, si no, volverá a lanzarlos.
	...
	class ListenerDados implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			int valorDado = numAleatorio[numeroDado - 1] + 1; // Esta variable almacenará el valor del dado que hemos
																// presionado
			if (tocaNumero) {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (tocaOperacion) {
				if (operacion.getName() == "Resta") {
					tocaSumar = false;
					tocaRestar = true;
					txtOperacion.setText(txtOperacion.getText() + "-"); // Añadimos el símbolo de la resta a la caja de
																		// texto
				} else if (operacion.getName() == "Suma") {
					tocaSumar = true;
					tocaRestar = false;
					txtOperacion.setText(txtOperacion.getText() + "+"); // Añadimos el símbolo de la suma a la caja de
																		// texto
				}
	...
	class ListenerBotonRepite implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (botonRepite.getText() == "REPITE JUGADA") {
				...
				// txtPuntuacion.setText("Puntuacion: 0"); //Reiniciamos la etiqueta que muestra
				// el resultado
	...
	class ListenerBotonVerificar implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// Alex JPanel panel = (JPanel) botonVerificar.getParent();
			// Alex JButton botonRepite = (JButton) panel.getComponents().
	...
	public void setLabelSaludo(String txt) {
		JLabel_Saludo.setText(txt); 
	}
